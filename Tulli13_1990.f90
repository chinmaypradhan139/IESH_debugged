...
!if (time.ge.593690) then
!   write(78,*) time, g(t,u)
!   write(79,*) time, lambda
!end if


if (g(t,u)>rnd) then
call nonadiabaticvector(t,u)
!write(80,*) time,acw(lambda(t),u,:) 
call hop1(t,u)
exit jloop
endif
enddo
...
**2+(2*Energy(lambda(t))/mass(1))-(2*Energy(u)/mass(1)))>0) then
...
! do x=1,Hi
!  do y=1,Hi
!   write(217,'(f12.8$)')time,Energy_hamil(x,y)
!  enddo
!   write(217,*)
! enddo
!end if
...
! do x=1,Hi
!  do y=1,Hi
!   write(217,'(f12.8$)')time,old_Energy_hamil(x,y)
!  enddo
!   write(217,*)
! enddo
!end if
